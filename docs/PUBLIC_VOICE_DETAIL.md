...
固定試聽在等音訊時顯示載入提示，仍可暫停；主動暫停造成的播放取消不會誤報失敗。每次播放分別識別要求，前一次或前一個聲線晚到的錯誤不能覆蓋目前狀態。真實音檔錯誤仍顯示提示，下次手動播放會重新載入；不自動重試或自動播放。
...
2026-09-08 補驗固定試聽的取消與重試：5 項新增互動測試先重現取消誤報、舊要求及舊聲線錯誤污染，再驗證修正、重載與重複要求；完整 74 項前端互動測試、164 項靜態檢查與 lint 通過。Chrome 152、Firefox 155、WebKit 26.5 以 HTTP WAV／Range 延遲回應，確認實際 `waiting`、暫停後不誤報錯誤、資料恢復後保持暫停及手動再播；1440／390／320px 無水平溢位。使用合成資料與 `/StoryVoice/` 正式建置，沒有呼叫合成服務或變更正式公開設定。
